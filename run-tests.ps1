...
function RunDockerMvnTest(
  ...
  if ($exitCode -ne 0) { [void]$failReasons.Add("docker/mvn exit code $exitCode") }
  ...
  if ($failReasons.Count -gt 0) {
    throw ("mvn test failed for {0}: {1} (see {2})" -f $src, ($failReasons -join "; "), $logPath)
  }
}
...
function PrintBackendSummary() {
  ...
  foreach ($r in $script:backendResults) {
    ...
    if ($null -ne $r.ExitCode) { Write-Host ("  docker exit: {0}" -f $r.ExitCode) }
...
function PrintBackendKpi() {
  ...
  Info "Backend KPI"
  Write-Host ("services: {0} | pass: {1} | fail: {2} | Results-sum total: {3} | wall elapsed: {4} s" -f $total, $passed, $failed, $testsSum, $elapsedSum)
...
if (-not $E2EOnly) {
  ...
  RunDockerMvnTest $workRoot $mvnImage "mvn -B -pl tasks-service -am test" "tasks-service" 10
  RunDockerMvnTest $workRoot $mvnImage "mvn -B -pl analytics-service -am test" "analytics-service" 10
  PrintBackendSummary
  PrintBackendKpi
}
...
if (-not $BackendOnly) {
  ...
  try {
    ...
    # would stick in $env: and break local "npm run dev" (still pointing at 8082). Use a temp --env-file only.
