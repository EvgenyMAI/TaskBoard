...
Для backend-части используется общий Maven cache (`.cache/m2`), что сокращает повторные скачивания зависимостей при последующих прогонах. Запуск Maven выполняется через `docker` с корректной фиксацией **кода выхода**; в логе дополнительно проверяются строка `BUILD FAILURE` и **сумма тестов** по блокам Surefire `[INFO] Results:` (с порогами по модулям), чтобы не получить ложноположительный «успех» при сбое.
