...
Интерфейс основных разделов выдержан в едином ритме: верхний hero-блок с тёмным градиентом и фиолетовой рамкой, аватар с инициалом, чипы с явными подписями показателей (число задач, проектов, непрочитанных уведомлений, выбранный период отчёта и т.д.), ниже — секции с маркером раздела. Страница аналитики использует тот же каркас: выбор интервала `from`/`to`, пресеты длительности, выгрузка CSV, сводные KPI, динамика периода, распределение статусов с переключением вида (кольцо или столбцы), топы проектов и исполнителей.
...
- фронтенд-вкладка «Аналитика»: те же отчётные срезы (KPI, динамика периода, статусы, топы, CSV), интерфейс унифицирован с экранами профиля и уведомлений.
