...
| Frontend | React 18, Vite, React Router; unit-тесты утилит и хуков — **Vitest** + Testing Library |
...
Основные разделы (проекты, задачи, аналитика, уведомления, профиль и карточки сущностей) выдержаны в **едином тёмном** визуальном стиле: градиентный фон, полупрозрачные карточки и панели, фиолетовые акценты и рамки, согласованная типографика. Вверху типичен блок-герой с аватаром-буквой и чипами с понятными подписями («Задач: N», «Проектов: N», «Непрочитанных: N», выбранный период отчёта и т.п.), ниже — секции с заголовками и маркерами разделов (◆).
...
- при **полном** прогоне (не **`-BackendOnly`** и не **`-E2EOnly`**) после успешного backend выполняет во **`frontend`**: **`npm install`** и **`npm run test`** (Vitest: утилиты, разбор уведомлений, `useClientPagination`);
...
Только фронтенд unit-тесты (без Maven и e2e): из каталога **`frontend`** — **`npm run test`**; в режиме разработки — **`npm run test:watch`**.

В конце полного прогона скрипт выводит краткий блок **Run summary** (Surefire + Vitest + E2E и путь к логам). Для `npm install` во время прогона заданы **`--no-audit --no-fund`**, чтобы не засорять консоль (при необходимости аудит зависимостей запускайте **`npm audit`** вручную).
...
2. После правок только утилит/хуков на фронте — `cd frontend; npm run test`
3. После правок UI / авторизации / уведомлений — `.\run-tests.ps1 -E2EOnly` или полный прогон
4. Перед merge — `.\run-tests.ps1`

Принципы: не размножать хрупкие e2e; тяжёлую подготовку данных в smoke выносить в API; в e2e проверять пользовательский поток, а не вёрстку; быстрые проверки выносить в Vitest.

### 9.5 CI/CD (GitHub Actions)

Файл **`.github/workflows/ci.yml`**: на `push` и `pull_request` — backend-тесты (`auth-service`; из корня `mvn … -pl tasks-service,analytics-service -am test`), затем job **e2e**: **`npm run test`** (Vitest) во **`frontend`**, поднятый **Docker Compose**, dev-сервер Vite и **Playwright**.
