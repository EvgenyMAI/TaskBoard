...
Помимо интеграционных сценариев в репозитории есть **юнит-тесты** (JUnit 5 + Mockito) на сервисы прикладного уровня и чистую логику отчётов — они проверяют граничные случаи без подъёма полного Spring-контекста и дополняют MockMvc-тесты.
...
Юнит-тесты (в тех же модулях, каталог `src/test/java/.../service/` и `.../controller/`): например, `NotificationApplicationService`, `TaskQueryService`, `ReportStatsCalculator`, `NotificationController`.
